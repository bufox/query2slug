...
class Test_Q2S_DB extends WP_UnitTestCase {
	...
	public function test_get_rule_by_slug(): void {
		...
		$rule = Q2S_DB::get_rule_by_slug( 'my-slug' );
		$this->assertNotNull( $rule );
		$this->assertSame( 'my-slug', $rule['slug'] );

		$this->assertNull( Q2S_DB::get_rule_by_slug( 'nonexistent' ) );
	}
	...
	public function test_get_rule_by_filters(): void {
		...
		// No match.
		$this->assertNull( Q2S_DB::get_rule_by_filters( array( 'product_cat' => 'hoodie' ) ) );
	}
	...
	public function test_slug_uniqueness(): void {
		...
		$this->assertSame( 'q2s_duplicate_slug', $result->get_error_code() );
	}
	...
	public function test_filter_uniqueness(): void {
		...
		$this->assertWPError( $result );
		$this->assertSame( 'q2s_duplicate_filters', $result->get_error_code() );
	}
	...
	public function test_delete_rule(): void {
		...
		$this->assertTrue( Q2S_DB::delete_rule( $id ) );
		$this->assertNull( Q2S_DB::get_rule( $id ) );
	}
	...
	public function test_get_rules_filtered_by_status(): void {
		Q2S_DB::save_rule( array( 'slug' => 'active-1', 'filters' => array( 'a' => '1' ), 'status' => 1 ) );
		Q2S_DB::save_rule( array( 'slug' => 'active-2', 'filters' => array( 'a' => '2' ), 'status' => 1 ) );
		Q2S_DB::save_rule( array( 'slug' => 'inactive-1', 'filters' => array( 'a' => '3' ), 'status' => 0 ) );

		$all      = Q2S_DB::get_rules();
		$active   = Q2S_DB::get_rules( 1 );
		$inactive = Q2S_DB::get_rules( 0 );
	...
	public function test_filters_all_empty_rejected(): void {
		...
		$this->assertWPError( $result );
		$this->assertSame( 'q2s_empty_filters', $result->get_error_code() );
	}

	public function test_bulk_activate(): void {
		$id1 = Q2S_DB::save_rule( array( 'slug' => 'bulk-a', 'filters' => array( 'a' => '1' ), 'status' => 0 ) );
		$id2 = Q2S_DB::save_rule( array( 'slug' => 'bulk-b', 'filters' => array( 'a' => '2' ), 'status' => 0 ) );
		$id3 = Q2S_DB::save_rule( array( 'slug' => 'bulk-c', 'filters' => array( 'a' => '3' ), 'status' => 0 ) );
		...
		$this->assertSame( 1, Q2S_DB::get_rule( $id3 )['status'] );
	}

	public function test_bulk_deactivate(): void {
		$id1 = Q2S_DB::save_rule( array( 'slug' => 'bulk-d', 'filters' => array( 'b' => '1' ), 'status' => 1 ) );
		$id2 = Q2S_DB::save_rule( array( 'slug' => 'bulk-e', 'filters' => array( 'b' => '2' ), 'status' => 1 ) );
	...
	public function test_bulk_delete(): void {
		$id1 = Q2S_DB::save_rule( array( 'slug' => 'bulk-f', 'filters' => array( 'c' => '1' ), 'status' => 1 ) );
		$id2 = Q2S_DB::save_rule( array( 'slug' => 'bulk-g', 'filters' => array( 'c' => '2' ), 'status' => 1 ) );
		$id3 = Q2S_DB::save_rule( array( 'slug' => 'bulk-h', 'filters' => array( 'c' => '3' ), 'status' => 1 ) );

		foreach ( array( $id1, $id2 ) as $id ) {
			Q2S_DB::delete_rule( $id );
		}

		$this->assertNull( Q2S_DB::get_rule( $id1 ) );
		$this->assertNull( Q2S_DB::get_rule( $id2 ) );
		// id3 should still exist.
		$this->assertNotNull( Q2S_DB::get_rule( $id3 ) );
	}
}
