...
class Test_Q2S_Rewrite extends WP_UnitTestCase {
	...
	public function test_query_var_registered(): void {
		$rewrite = new Q2S_Rewrite();
		$vars    = $rewrite->register_query_var( array() );

		$this->assertContains( 'q2s_slug', $vars );
	}

	public function test_get_prefix_default(): void {
		delete_option( 'q2s_prefix' );
		$this->assertSame( 'lp', Q2S_Rewrite::get_prefix() );
	}

	public function test_get_prefix_custom(): void {
		update_option( 'q2s_prefix', 'go' );
		$this->assertSame( 'go', Q2S_Rewrite::get_prefix() );
	}
	...
	public function test_resolve_slug_injects_filters(): void {
		...
		// Simulate a WP object with q2s_slug set.
		$wp = new WP();
		$wp->query_vars = array( 'q2s_slug' => 'disney-tshirts' );

		$rewrite->resolve_slug( $wp );

		$this->assertArrayNotHasKey( 'q2s_slug', $wp->query_vars );
	...
	public function test_resolve_slug_inactive_rule_ignored(): void {
		...
		$rewrite = new Q2S_Rewrite();
		$wp      = new WP();
		$wp->query_vars = array( 'q2s_slug' => 'inactive-rule' );
	...
	public function test_resolve_slug_unknown_slug_returns_404(): void {
		$rewrite = new Q2S_Rewrite();
		$wp      = new WP();
		$wp->query_vars = array( 'q2s_slug' => 'does-not-exist' );
	...
	public function test_prefix_change_affects_resolution(): void {
		...
		$rewrite = new Q2S_Rewrite();
		$wp      = new WP();
		$wp->query_vars = array( 'q2s_slug' => 'prefix-test' );
		$rewrite->resolve_slug( $wp );
