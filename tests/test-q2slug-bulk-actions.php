...
class Test_Q2SLUG_Bulk_Actions extends WP_UnitTestCase {
	...
	private function do_bulk_action( string $action, array $rule_ids ): void {
		...
		$_POST['action']       = $action;
		$_POST['q2slug_rule_ids'] = $rule_ids;
	...
	public function test_bulk_deactivate(): void {
		$id1 = Q2SLUG_DB::save_rule( array( 'slug' => 'bulk-a', 'filters' => array( 'cat' => 'a' ), 'status' => 1 ) );
		$id2 = Q2SLUG_DB::save_rule( array( 'slug' => 'bulk-b', 'filters' => array( 'cat' => 'b' ), 'status' => 1 ) );
	...
	public function test_bulk_activate(): void {
		$id1 = Q2SLUG_DB::save_rule( array( 'slug' => 'bulk-c', 'filters' => array( 'cat' => 'c' ), 'status' => 0 ) );
		$id2 = Q2SLUG_DB::save_rule( array( 'slug' => 'bulk-d', 'filters' => array( 'cat' => 'd' ), 'status' => 0 ) );
	...
	public function test_bulk_delete(): void {
		$id1 = Q2SLUG_DB::save_rule( array( 'slug' => 'bulk-e', 'filters' => array( 'cat' => 'e' ), 'status' => 1 ) );
		$id2 = Q2SLUG_DB::save_rule( array( 'slug' => 'bulk-f', 'filters' => array( 'cat' => 'f' ), 'status' => 1 ) );
		$keep = Q2SLUG_DB::save_rule( array( 'slug' => 'bulk-keep', 'filters' => array( 'cat' => 'keep' ), 'status' => 1 ) );

		$this->do_bulk_action( 'delete', array( $id1, $id2 ) );

		$this->assertNull( Q2SLUG_DB::get_rule( $id1 ) );
		$this->assertNull( Q2SLUG_DB::get_rule( $id2 ) );
		$this->assertNotNull( Q2SLUG_DB::get_rule( $keep ) );
	}

	public function test_bulk_no_action_loads_items(): void {
		Q2SLUG_DB::save_rule( array( 'slug' => 'no-action', 'filters' => array( 'cat' => 'x' ), 'status' => 1 ) );
	...
	public function test_bulk_requires_capability(): void {
		$id = Q2SLUG_DB::save_rule( array( 'slug' => 'cap-test', 'filters' => array( 'cat' => 'z' ), 'status' => 1 ) );
		...
		$_REQUEST['_wpnonce']  = $nonce;
		$_POST['action']       = 'delete';
		$_POST['q2slug_rule_ids'] = array( $id );
		$_POST['_wpnonce']     = $nonce;
	...
	public function test_bulk_empty_ids_does_nothing(): void {
		$id = Q2SLUG_DB::save_rule( array( 'slug' => 'empty-ids', 'filters' => array( 'cat' => 'q' ), 'status' => 1 ) );
		...
		$_REQUEST['_wpnonce']  = $nonce;
		$_POST['action']       = 'delete';
		$_POST['q2slug_rule_ids'] = array();
		$_POST['_wpnonce']     = $nonce;

		$table->prepare_items();

		$this->assertNotNull( Q2SLUG_DB::get_rule( $id ) );
	}
}
