 ...
 */
class Q2SLUG_Redirect_Caught extends Exception {
...
class Test_Q2SLUG_Redirect extends WP_UnitTestCase {
	...
	public function set_up(): void {
		...
		add_filter( 'wp_redirect', function ( string $location, int $status ): string {
			throw new Q2SLUG_Redirect_Caught( $location, $status );
		}, 1, 2 );
	}
	 ...
	 */
	private function do_redirect( array $get_params ): Q2SLUG_Redirect_Caught {
		$_GET    = $get_params;
		$redirect = new Q2SLUG_Redirect();

		try {
			$redirect->maybe_redirect();
		} catch ( Q2SLUG_Redirect_Caught $e ) {
			return $e;
		}
	...
	public function test_tracking_params_list(): void {
		...
		$this->assertContains( 'utm_source', Q2SLUG_Redirect::TRACKING_PARAMS );
		$this->assertContains( 'fbclid', Q2SLUG_Redirect::TRACKING_PARAMS );
		$this->assertContains( 'gclid', Q2SLUG_Redirect::TRACKING_PARAMS );
	}

	public function test_filter_hash_order_independent(): void {
		$hash_a = Q2SLUG_DB::compute_filter_hash( array( 'cat' => 't-shirt', 'tag' => 'disney' ) );
		$hash_b = Q2SLUG_DB::compute_filter_hash( array( 'tag' => 'disney', 'cat' => 't-shirt' ) );

		$this->assertSame( $hash_a, $hash_b );
	}

	public function test_filter_hash_different_for_different_filters(): void {
		$hash_a = Q2SLUG_DB::compute_filter_hash( array( 'cat' => 't-shirt' ) );
		$hash_b = Q2SLUG_DB::compute_filter_hash( array( 'cat' => 'hoodie' ) );
	...
	public function test_inactive_rule_not_matched_for_redirect(): void {
		...
		$rule = Q2SLUG_DB::get_rule_by_filters( array( 'cat' => 'test' ) );
